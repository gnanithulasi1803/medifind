...
function killSession(req) {
  ...
  for (const part of raw.split(';')) {
    const t = part.trim();
    if (t.startsWith('sid=')) { delete SESSIONS[t.slice(4)]; break; }
  }
}
...
async function handleAPI(req, res, pname) {
  ...
  if (pname === '/api/login' && M === 'POST') {
    ...
    if (!email || !pass) return json(res, 400, { error:'Email and password required.' });

    try {
      if (role === 'ADMIN') {
        if (email === 'admin' && pass === 'admin123') {
          const sid = makeSession({ role:'ADMIN', name:'Administrator', id:0 });
          res.writeHead(200, { 'Content-Type':'application/json', 'Set-Cookie':`sid=${sid};Path=/;HttpOnly;Max-Age=86400` });
          return res.end(JSON.stringify({ success:true, role:'ADMIN', name:'Administrator' }));
        }
        return json(res, 401, { error:'Wrong admin credentials.' });
      }
      if (role === 'PHARMACY') {
        const rows = await q('SELECT * FROM pharmacies WHERE email=? AND password=?', [email, pass]);
        if (!rows.length) return json(res, 401, { error:'Wrong email or password.' });
        if (!rows[0].is_active) return json(res, 401, { error:'Account deactivated by admin.' });
        const p = rows[0];
        const sid = makeSession({ role:'PHARMACY', name:p.pharmacy_name, id:p.id, email:p.email });
        res.writeHead(200, { 'Content-Type':'application/json', 'Set-Cookie':`sid=${sid};Path=/;HttpOnly;Max-Age=86400` });
        return res.end(JSON.stringify({ success:true, role:'PHARMACY', name:p.pharmacy_name, id:p.id }));
      }
      const rows = await q('SELECT * FROM users WHERE email=? AND password=?', [email, pass]);
      if (!rows.length) return json(res, 401, { error:'Wrong email or password.' });
      const u = rows[0];
      const sid = makeSession({ role:'USER', name:u.full_name, id:u.id, email:u.email });
      res.writeHead(200, { 'Content-Type':'application/json', 'Set-Cookie':`sid=${sid};Path=/;HttpOnly;Max-Age=86400` });
      return res.end(JSON.stringify({ success:true, role:'USER', name:u.full_name, id:u.id }));
    } catch(e) { return json(res, 500, { error:'DB error: '+e.message }); }
  }

  // ── /api/register/user ──
  if (pname === '/api/register/user' && M === 'POST') {
    const b = await readBody(req);
    if (!b.fullName||!b.email||!b.password) return json(res, 400, { error:'Name, email, password required.' });
    try {
      const ex = await q('SELECT id FROM users WHERE email=?', [b.email]);
      if (ex.length) return json(res, 409, { error:'Email already registered.' });
      const r = await q(
        'INSERT INTO users (full_name,email,password,phone,address,latitude,longitude) VALUES (?,?,?,?,?,?,?)',
        [b.fullName.trim(), b.email.trim(), b.password, b.phone||'', b.address||'', parseFloat(b.latitude)||0, parseFloat(b.longitude)||0]
      );
  ...
  if (pname === '/api/register/pharmacy' && M === 'POST') {
    const b = await readBody(req);
    if (!b.ownerName||!b.pharmacyName||!b.email||!b.password) return json(res, 400, { error:'All fields required.' });
    try {
      ...
      const r = await q(
        ...
        [b.ownerName.trim(), b.pharmacyName.trim(), b.email.trim(), b.password, b.phone||'', b.address||'', b.city||'', parseFloat(b.latitude)||0, parseFloat(b.longitude)||0]
      );
      return json(res, 200, { success:true, id:r.insertId });
    } catch(e) { return json(res, 500, { error:e.message }); }
  }
  ...
  if (pname === '/api/search' && M === 'GET') {
    ...
    try {
      ...
      let results = rows.map(r => ({ ...r, distanceKm:(uLat&&uLng)?haversine(uLat,uLng,+r.plat,+r.plng):0 }));
      if (uLat&&uLng) {
  ...
  if (pname === '/api/cart/checkout' && M === 'POST') {
    ...
    if (!s||s.role!=='USER') return json(res, 401, { error:'Login required.' });
    const b = await readBody(req);
    // b.items = [{ medicineId, quantity, deliveryType, deliveryAddress }]
    if (!b.items||!b.items.length) return json(res, 400, { error:'Cart is empty.' });

    const placed = [], failed = [];
    for (const item of b.items) {
      try {
        const meds = await q('SELECT * FROM medicines WHERE id=?', [parseInt(item.medicineId)]);
        if (!meds.length) { failed.push({ id:item.medicineId, reason:'Medicine not found' }); continue; }
        const med = meds[0];
        const qty = parseInt(item.quantity)||1;
        if (med.stock_quantity < qty) { failed.push({ id:item.medicineId, name:med.medicine_name, reason:'Insufficient stock' }); continue; }
        ...
        const r = await q(
          ...
          [s.id, med.pharmacy_id, med.id, qty, total, item.deliveryType||'PICKUP', item.deliveryAddress||'', 'PENDING']
        );
        await q('UPDATE medicines SET stock_quantity=stock_quantity-? WHERE id=?', [qty, med.id]);
        ...
        const [ord] = await q(
          `SELECT o.*,m.medicine_name,m.brand,m.unit,
           ...
           WHERE o.id=?`, [r.insertId]
        );

        // Notifications
        await createNotification(s.id, null, `Your order #${r.insertId} for ${med.medicine_name} has been placed.`, 'ORDER_PLACED');
        await createNotification(null, med.pharmacy_id, `New order #${r.insertId} from ${s.name} for ${med.medicine_name}.`, 'NEW_ORDER');
        ...
        placed.push({ orderId:r.insertId, medicineName:med.medicine_name, total });
      } catch(e) {
    ...
    try {
      const rows = await q(
        `SELECT o.id,o.quantity,o.total_price,o.delivery_type,o.delivery_address,o.status,o.order_date,
         ...
         JOIN medicines m ON o.medicine_id=m.id
         JOIN pharmacies p ON o.pharmacy_id=p.id
         WHERE o.user_id=? ORDER BY o.order_date DESC`, [s.id]
      );
  ...
  if (pname === '/api/orders/pharmacy' && M === 'GET') {
    const s = getSession(req);
    if (!s||s.role!=='PHARMACY') return json(res, 401, { error:'Login required.' });
    try {
      const rows = await q(
        `SELECT o.id,o.quantity,o.total_price,o.delivery_type,o.delivery_address,o.status,o.order_date,
         ...
         JOIN medicines m ON o.medicine_id=m.id
         JOIN users u ON o.user_id=u.id
         WHERE o.pharmacy_id=? ORDER BY o.order_date DESC`, [s.id]
      );
      return json(res, 200, rows);
    } catch(e) { return json(res, 500, { error:e.message }); }
  }
  ...
  if (pname === '/api/order/status' && M === 'POST') {
    const s = getSession(req);
    if (!s||(s.role!=='PHARMACY'&&s.role!=='ADMIN')) return json(res, 401, { error:'Not authorised.' });
    const b = await readBody(req);
    const allowed = ['CONFIRMED','REJECTED','DISPATCHED','DELIVERED'];
    if (!b.orderId||!allowed.includes(b.status)) return json(res, 400, { error:'Invalid request.' });
    try {
      ...
      if (ord) {
        ...
        await createNotification(ord.user_id, null, statusMsg[b.status], 'ORDER_UPDATE');
  ...
  if (pname === '/api/notifications' && M === 'GET') {
    ...
    try {
      let rows;
      if (s.role === 'USER')
        rows = await q('SELECT * FROM notifications WHERE user_id=? ORDER BY created_at DESC LIMIT 20', [s.id]);
      else if (s.role === 'PHARMACY')
        rows = await q('SELECT * FROM notifications WHERE pharmacy_id=? ORDER BY created_at DESC LIMIT 20', [s.id]);
      else
        rows = await q('SELECT * FROM notifications ORDER BY created_at DESC LIMIT 30');
      return json(res, 200, rows);
    } catch(e) { return json(res, 500, { error:e.message }); }
  }
  ...
  if (pname === '/api/notifications/readall' && M === 'POST') {
    ...
    try {
      if (s.role === 'USER')
        ...
        await q('UPDATE notifications SET is_read=1 WHERE pharmacy_id=?', [s.id]);
      return json(res, 200, { success:true });
    } catch(e) { return json(res, 500, { error:e.message }); }
  }
  ...
  if (pname === '/api/medicines' && M === 'GET') {
    const s = getSession(req);
    if (!s||s.role!=='PHARMACY') return json(res, 401, { error:'Login required.' });
    try { return json(res, 200, await q('SELECT * FROM medicines WHERE pharmacy_id=? ORDER BY medicine_name', [s.id])); }
    catch(e) { return json(res, 500, { error:e.message }); }
  }
  ...
  if (pname === '/api/medicines/add' && M === 'POST') {
    ...
    if (!s||s.role!=='PHARMACY') return json(res, 401, { error:'Login required.' });
    const b = await readBody(req);
    if (!b.medicineName||b.price===undefined||b.stockQuantity===undefined)
    ...
    try {
      const r = await q(
        ...
        [s.id, b.medicineName.trim(), b.brand||'', b.category||'General', parseFloat(b.price), parseInt(b.stockQuantity), b.unit||'Tablet', b.requiresPrescription?1:0, b.deliveryAvailable!==false?1:0, b.description||'']
      );
      return json(res, 200, { success:true, id:r.insertId });
    } catch(e) { return json(res, 500, { error:e.message }); }
  }
  ...
  if (pname === '/api/medicines/update' && M === 'POST') {
    ...
    try {
      await q(
        ...
        [b.medicineName.trim(), b.brand||'', b.category||'General', parseFloat(b.price), parseInt(b.stockQuantity), b.unit||'Tablet', b.requiresPrescription?1:0, b.deliveryAvailable!==false?1:0, b.description||'', +b.id, s.id]
      );
      return json(res, 200, { success:true });
    } catch(e) { return json(res, 500, { error:e.message }); }
  }
  ...
  if (pname === '/api/medicines/delete' && M === 'POST') {
    ...
    if (!s||s.role!=='PHARMACY') return json(res, 401, { error:'Login required.' });
    const b = await readBody(req);
    try { await q('DELETE FROM medicines WHERE id=? AND pharmacy_id=?', [+b.id, s.id]); return json(res, 200, { success:true }); }
    catch(e) { return json(res, 500, { error:e.message }); }
  }
  ...
  if (pname.startsWith('/api/admin/')) {
    const s = getSession(req);
    if (!s||s.role!=='ADMIN') return json(res, 401, { error:'Admin access required.' });

    if (pname === '/api/admin/stats') {
      const [[u],[p],[m],[o]] = await Promise.all([
        q('SELECT COUNT(*) c FROM users'), q('SELECT COUNT(*) c FROM pharmacies'),
        q('SELECT COUNT(*) c FROM medicines'), q('SELECT COUNT(*) c FROM orders')
      ]);
      return json(res, 200, { users:u.c, pharmacies:p.c, medicines:m.c, orders:o.c });
    }
    if (pname === '/api/admin/users')
      return json(res, 200, await q('SELECT id,full_name,email,phone,address,created_at FROM users ORDER BY created_at DESC'));
    if (pname === '/api/admin/pharmacies')
      return json(res, 200, await q('SELECT id,pharmacy_name,owner_name,email,city,phone,is_active FROM pharmacies ORDER BY pharmacy_name'));
    if (pname === '/api/admin/medicines')
      return json(res, 200, await q('SELECT m.*,p.pharmacy_name FROM medicines m JOIN pharmacies p ON m.pharmacy_id=p.id ORDER BY p.pharmacy_name,m.medicine_name'));
    if (pname === '/api/admin/orders')
      return json(res, 200, await q(`SELECT o.id,o.quantity,o.total_price,o.delivery_type,o.status,o.order_date,m.medicine_name,m.brand,p.pharmacy_name,u.full_name AS user_name FROM orders o JOIN medicines m ON o.medicine_id=m.id JOIN pharmacies p ON o.pharmacy_id=p.id JOIN users u ON o.user_id=u.id ORDER BY o.order_date DESC`));
    if (pname === '/api/admin/pharmacy/toggle' && M === 'POST') {
      const b = await readBody(req);
      await q('UPDATE pharmacies SET is_active=? WHERE id=?', [b.active?1:0, +b.id]);
      return json(res, 200, { success:true });
    }
  }
...
//  STATIC FILES
// ════════════════════════════════════════════════════════
const MIME = { '.html':'text/html;charset=utf-8','.css':'text/css','.js':'text/javascript','.json':'application/json','.png':'image/png','.jpg':'image/jpeg','.ico':'image/x-icon','.svg':'image/svg+xml' };
...
const server = http.createServer(async (req, res) => {
  ...
  if (pname.startsWith('/api/')) {
    try { await handleAPI(req, res, pname); }
    catch(e) { console.error('API Error:', e); json(res, 500, { error:e.message }); }
    return;
  }

  const fp = path.join(__dirname, pname === '/' ? 'index.html' : pname.split('?')[0]);
